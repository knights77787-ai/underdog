...
- Git에는 `Backend/App/`(대문자 A)로 올라가 있는데, 코드에서 `from app.main import app`(소문자)을 쓰면 Linux에서는 `app` 패키지를 찾지 못합니다.
...
**증상:** `https://api.lumen.ai.kr/` 접속 시 404.
...
- OAuth는 **리다이렉트 URL**을 배포 도메인(예: `https://api.lumen.ai.kr`)으로 Google/Kakao 콘솔에 등록해야 합니다.
