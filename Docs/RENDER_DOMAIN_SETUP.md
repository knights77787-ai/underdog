...
| **Start Command** | `uvicorn main:app --host 0.0.0.0 --port $PORT` 또는 `cd Backend && uvicorn main:app --host 0.0.0.0 --port $PORT` |
| **Pre-Deploy Command** | *(비움)* |

- **레포 루트에 `main.py` 있음:** `from Backend.App.main import app` 만 하는 진입점. 이걸 쓰면 Start Command에 **`cd Backend` 없이** `uvicorn main:app --host 0.0.0.0 --port $PORT` 만 넣어도 됨.
- Build Command는 그대로 **`cd Backend && pip install -r requirements.txt`** (의존성은 Backend 기준).
- **"Could not import module 'main'"** 나오면: Start Command에 `cd Backend &&` 가 없다면 반드시 **레포 루트**에서 실행 중이어야 함 (Root Directory 비움). 레포 루트의 `main.py`가 있으므로 `uvicorn main:app` 만 써도 됨. 포트는 **`$PORT`** 로 두세요 (10000 고정 금지).
