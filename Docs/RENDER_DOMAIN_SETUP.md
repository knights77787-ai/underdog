...
- **반드시 `cd Backend && uvicorn app.main:app ...` 사용:** Render(Linux)는 대소문자 구분. 루트에서 `uvicorn main:app`으로 실행하면 `Backend.App`/`App` 경로 때문에 `ModuleNotFoundError: No module named 'App'` 가 납니다. **Backend** 를 작업 디렉터리로 두고 **`app.main:app`** 으로 실행하면 해결됩니다.
